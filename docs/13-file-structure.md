...
│
├── env/                                     # 环境服务（每个执行环境一个 AppService）
│   ├── __init__.py
│   ├── local/
│   │   ├── __init__.py
│   │   └── service.py                       # LocalToolService(AppService) + LocalProtocol
│   ├── docker/
│   │   ├── __init__.py
│   │   └── service.py                       # DockerToolService(AppService) + DockerProtocol
│   └── ssh/
│       ├── __init__.py
│       └── service.py                       # SSHToolService(AppService) + SSHProtocol
│
...
### Env — 环境服务 (8 files, ~200 lines)

| File | Lines | Purpose |
|------|-------|---------|
| `env/__init__.py` | ~5 | Exports |
| `env/local/__init__.py` | ~5 | Exports |
| `env/local/service.py` | ~50 | LocalToolService(AppService): 持有 LocalProtocol，委托 execute/read_file/write_file |
| `env/docker/__init__.py` | ~5 | Exports |
| `env/docker/service.py` | ~50 | DockerToolService(AppService): 持有 DockerProtocol，委托 execute/upload/download |
| `env/ssh/__init__.py` | ~5 | Exports |
| `env/ssh/service.py` | ~50 | SSHToolService(AppService): 持有 SSHProtocol，委托 execute/upload/download |
